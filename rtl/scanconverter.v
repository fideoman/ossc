...
`define LINEBUF_WRITE_INDEX     (linebuf_4xmode ? linebuf_write_idx : {linebuf_write_idx[1], hcnt_1x_sample_offset[10]})
...
wire [2:0] row_act;
wire [2:0] col_act;
reg [2:0] col_pp1, row_pp1, col_pp2, row_pp2;
reg [2:0] col_2x, col_3x;
...
//reg [7:0] R_1x, G_1x, B_1x, R_2x, G_2x, B_2x, R_3x, G_3x, B_3x, R_4x, G_4x, B_4x, R_5x, G_5x, B_5x, R_pp1, G_pp1, B_pp1, R_pp2, G_pp2, B_pp2;
//wire [7:0] R_lbuf, G_lbuf, B_lbuf;
//wire [7:0] R_act, G_act, B_act;
reg [3:0] X_MASK_BR;
...
// alignment for write clock skip
reg [11:0] hcnt_1x_sample_offset;
reg [2:0] hcnt_1x_sample_cnt;
    ...
    `V_MULTMODE_2X: begin
        ...
        case (H_MULTMODE)
            ...
            `H_MULTMODE_OPTIMIZED: begin
                ...
                //col_act = hcnt_2x[0] << 1;
                col_act = {1'b0, col_2x, 1'b0};
                row_act = {1'b0, line_out_idx_2x[0], 1'b0};
            end
        endcase
    end
    `V_MULTMODE_3X: begin
...
// linebuf linebuf_rgb (
    ...
    // .wraddress ( hcnt_1x_sample_offset + (line_idx << 11) ),
    // .wrclock ( pclk_1x ),
    // .wren ( H_MULTMODE != `H_MULTMODE_OPTIMIZED || hcnt_1x_sample_cnt == H_OPT_SAMPLE_SEL ),
...
linebuf_1k linebuf_rgb[`LINEBUF_NUM] (
    ...
    .rdclock ( linebuf_rdclock ),
    .wraddress ( hcnt_1x_sample_offset[9:0] ),
    .wrclock ( pclk_1x ),
    .wren ( linebuf_wren ),
    .q ( linebuf_o )
);
...
// filter filter_inst(
    ...
    // .mode(linebuf_4xmode ? X_FILTER : 0),
    // .str(X_FILTERSTR),
    // .col(col_act),
    // .row(row_act),
    // .col_id(col_id_act),
...
filter filter_inst(
    ...
    .hsync_in(HSYNC_pp2),
    .vsync_in(VSYNC_pp2),
    .dataenable_in(DE_pp2),
    .mask_enable_in(mask_enable_pp2),
    ...
    .col_id_out(col_id_fout),
    .line_id_out(line_id_fout),
    .mask_enable_out(mask_enable_fout),
...
begin
    ...
    // HSYNC_pp2 <= HSYNC_fout;
    // VSYNC_pp2 <= VSYNC_fout;
    // DE_pp2 <= DE_fout;
    // line_id_pp2 <= line_id_fout;
    // col_id_pp2 <= col_id_fout;
    // mask_enable_pp2 <= mask_enable_fout;
    ...
    R_pp3 <= apply_scanlines(V_SCANLINEMODE, data_fout[`RVAL], X_SCANLINESTR, V_SCANLINEID, line_id_fout, col_id_fout, FID_1x);
    G_pp3 <= apply_scanlines(V_SCANLINEMODE, data_fout[`GVAL], X_SCANLINESTR, V_SCANLINEID, line_id_fout, col_id_fout, FID_1x);
    B_pp3 <= apply_scanlines(V_SCANLINEMODE, data_fout[`BVAL], X_SCANLINESTR, V_SCANLINEID, line_id_fout, col_id_fout, FID_1x);
    HSYNC_pp3 <= HSYNC_fout;
    VSYNC_pp3 <= VSYNC_fout;
    DE_pp3 <= DE_fout;
    mask_enable_pp3 <= mask_enable_fout;
...
begin
    if (!reset_n) begin
        ...
        if (`HSYNC_LEADING_EDGE) begin
            ...
            hcnt_1x_sample_offset <= 0;
            hcnt_1x_sample_cnt <= 0;
            ...
            if (H_MULTMODE != `H_MULTMODE_OPTIMIZED || hcnt_1x_sample_cnt == H_OPT_SAMPLE_MULT-1'b1) begin
                hcnt_1x_sample_offset <= hcnt_1x_sample_offset + 1'b1;
                hcnt_1x_sample_cnt <= 0;
            end
            else begin
                hcnt_1x_sample_cnt <= hcnt_1x_sample_cnt + 1'b1;
            end
        end
        ...
        if (frame_change) begin
            ...
            X_FILTERSTR <= extra_info[10:8];
            X_FILTER <= extra_info[12:11];
        end
...
begin
    if (!reset_n) begin
        ...
        if ((pclk_1x == 1'b0) & (line_change | frame_change)) begin  //aligned with posedge of pclk_1x
            hcnt_2x <= 0;
            hcnt_2x_opt <= 0; // H_OPT_SAMPLE_SEL;
            hcnt_2x_opt_ctr <= 0;
            line_out_idx_2x <= 0;
            col_2x <= 0;
            if (frame_change)
            ...
            if (hcnt_2x >= H_OPT_STARTOFF) begin
                if (hcnt_2x_opt_ctr == H_OPT_SCALE-1'b1) begin
                    hcnt_2x_opt <= hcnt_2x_opt + 1'b1; // H_OPT_SAMPLE_MULT;
                    hcnt_2x_opt_ctr <= 0;
                end else
    ...
    if (!reset_n) begin
        ...
        if ((pclk_3x_cnt == 0) & (line_change | frame_change)) begin  //aligned with posedge of pclk_1x
            if (!(frame_change & (FID_cur == `FID_ODD))) begin
                hcnt_3x <= 0;
                hcnt_3x_opt <= 0; // H_OPT_SAMPLE_SEL;
                hcnt_3x_opt_ctr <= 0;
                line_out_idx_3x <= 0;
                col_3x <= 0;
            end
            ...
            if (hcnt_3x >= H_OPT_STARTOFF) begin
                if (hcnt_3x_opt_ctr == H_OPT_SCALE-1'b1) begin
                    hcnt_3x_opt <= hcnt_3x_opt + 1'b1; // H_OPT_SAMPLE_MULT;
                    hcnt_3x_opt_ctr <= 0;
                end else
        ...
        if ((pclk_4x_cnt == 0) & (line_change | frame_change)) begin  //aligned with posedge of pclk_1x
            ...
            if (hcnt_4x >= H_OPT_STARTOFF) begin
                if (hcnt_4x_opt_ctr == H_OPT_SCALE-1'b1) begin
                    hcnt_4x_opt <= hcnt_4x_opt + 1'b1; // H_OPT_SAMPLE_MULT;
                    hcnt_4x_opt_ctr <= 0;
                end else
        ...
        if ((pclk_5x_cnt == 0) & (line_change | frame_change)) begin  //aligned with posedge of pclk_1x
            ...
            if (hcnt_5x >= H_OPT_STARTOFF) begin
                if (hcnt_5x_opt_ctr == H_OPT_SCALE-1'b1) begin
                    hcnt_5x_opt <= hcnt_5x_opt + 1'b1; // H_OPT_SAMPLE_MULT;
                    hcnt_5x_opt_ctr <= 0;
                end else
...
// linebuffer
always @(*) begin
    prev_act = linebuf_o[linebuf_read_idx_m1];
    { R_lbuf, G_lbuf, B_lbuf } = linebuf_o[`LINEBUF_READ_INDEX];
    next_act = linebuf_o[linebuf_read_idx_a1];
