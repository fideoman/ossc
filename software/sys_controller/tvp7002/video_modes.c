        ...
        if ((typemask & mode_type) && (target_lm & video_modes[i].flags) && (progressive == !(video_modes[i].flags & MODE_INTERLACED)) && (totlines <= (video_modes[i].v_total+LINECNT_MAX_TOLERANCE))) {
            ...
            switch (cm.target_lm) {
                ...
                case MODE_L2:
                    ...
                    if ((!cm.cc.vga_ilace_fix) && ((video_modes[i].group == GROUP_240P) || (video_modes[i].group == GROUP_384P) || (video_modes[i].group == GROUP_480I) || (video_modes[i].group == GROUP_DTV480P) || (video_modes[i].group == GROUP_VGA480P))) {
